   ...
   public:
      ...
      */
      inline bool read( char* d, size_t s ) {
        uosio::check( size_t(_end - _pos) >= (size_t)s, "read" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        uosio::check( _end - _pos >= (int32_t)s, "write" );
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const uosio::binary_extension<T>& be) {
  ds << be.value_or();
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, uosio::binary_extension<T>& be) {
  if( ds.remaining() ) {
     T val;
     ds >> val;
     be.emplace(val);
...
void deserialize(datastream<Stream>& ds, std::variant<Ts...>& var, int i) {
   if constexpr (I < std::variant_size_v<std::variant<Ts...>>) {
      ...
      uosio::check(false, "invalid variant index");
   }
}
...
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const uosio::symbol_code sym_code) {
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, uosio::symbol_code& sym_code) {
  uint64_t raw = 0;
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const uosio::symbol sym) {
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, uosio::symbol& sym) {
  uint64_t raw = 0;
  ds.read((char*)&raw, sizeof(raw));
  sym = symbol(raw);
  return ds;
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const ::uosio::ignore_wrapper<T>& val) {
  ds << val.value;
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const ::uosio::ignore<T>& val) {
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, ::uosio::ignore<T>) {
  return ds;
}
 ...
 *
 *  @brief Serialize an uosio::public_key
 *  @param ds - The stream to write
 *  @param pubkey - The value to serialize
 *  @tparam Stream - Type of datastream buffer
 *  @return datastream<Stream>& - Reference to the datastream
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const uosio::public_key& pubkey) {
   ds << pubkey.type;
 ...
 *
 *  @brief Deserialize an uosio::public_key
 *  @param ds - The stream to read
 *  @param pubkey - The destination for deserialized value
 *  @tparam Stream - Type of datastream buffer
 *  @return datastream<Stream>& - Reference to the datastream
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, uosio::public_key& pubkey) {
   ds >> pubkey.type;
 ...
 *
 *  @brief Serialize an uosio::signature
 *  @param ds - The stream to write
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const uosio::signature& sig) {
   ds << sig.type;
 ...
 *
 *  @brief Deserialize an uosio::signature
 *  @param ds - The stream to read
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, uosio::signature& sig) {
   ds >> sig.type;
...
DataStream& operator >> ( DataStream& ds, T (&v)[N] ) {
   unsigned_int s;
   ds >> s;
   uosio::check( N == s.value, "T[] size and unpacked size don't match");
   for( uint32_t i = 0; i < N; ++i )
      ds >> v[i];
   return ds;
}
