...
#warning "<uosiolib/binary_extension.hpp> is deprecated use <uosio/binary_extension.hpp>"
 ...
 namespace uosio {
   ...
   class binary_extension {
      public:
         ...
         constexpr T& value()& {
            if (!_has_value) {
               uosio_assert(false, "cannot get value of empty binary_extension");
            }
            return _get();
         }
          /** get the contained value */
         ...
         **/
         ...
         friend inline DataStream& operator<<(DataStream& ds, const uosio::binary_extension<T>& be) {
            ds << be.value_or();
            return ds;
         }
         ...
         /**
           ...
           */
         template<typename DataStream>
         friend inline DataStream& operator>>(DataStream& ds, uosio::binary_extension<T>& be) {
            if( ds.remaining() ) {
               T val;
               ds >> val;
               be.emplace(val);
