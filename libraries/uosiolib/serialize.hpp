...
#warning "<uosiolib/serialize.hpp> is deprecated use <uosio/serialize.hpp>"
#define UOSLIB_REFLECT_MEMBER_OP( r, OP, elem ) \
  OP t.elem
 ...
 */
#define UOSLIB_SERIALIZE( TYPE,  MEMBERS ) \
 template<typename DataStream> \
 friend DataStream& operator << ( DataStream& ds, const TYPE& t ){ \
    return ds BOOST_PP_SEQ_FOR_EACH( UOSLIB_REFLECT_MEMBER_OP, <<, MEMBERS );\
 }\
 template<typename DataStream> \
 ...
 *  @param MEMBERS - a sequence of member names.  (field1)(field2)(field3)
 */
#define UOSLIB_SERIALIZE_DERIVED( TYPE, BASE, MEMBERS ) \
 template<typename DataStream> \
 friend DataStream& operator << ( DataStream& ds, const TYPE& t ){ \
