...
namespace uosio {
   ...
   public:
      ...
      {
         if( str.size() > 13 ) {
            uosio::check( false, "string is too long to be a valid name" );
         }
         if( str.empty() ) {
            return;
         }
         ...
         if( str.size() == 13 ) {
            ...
            if( v > 0x0Full ) {
               uosio::check(false, "thirteenth character in name cannot be a letter that comes after j");
            }
            value |= v;
         }
      }
      ...
      static constexpr uint8_t char_to_value( char c ) {
         if( c == '.')
            ...
            uosio::check( false, "character is not in allowed character set for names" );
...
inline constexpr uosio::name operator""_n() {
   constexpr auto x = uosio::name{std::string_view{uosio::detail::to_const_char_arr<Str...>::value, sizeof...(Str)}};
   return x;
}
