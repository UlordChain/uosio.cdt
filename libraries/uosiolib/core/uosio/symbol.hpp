...
namespace uosio {
   ...
   */
   ...
   public:
      ...
      {
         if( str.size() > 7 ) {
            uosio::check( false, "string is too long to be a valid symbol_code" );
         }
         for( auto itr = str.rbegin(); itr != str.rend(); ++itr ) {
            if( *itr < 'A' || *itr > 'Z') {
               uosio::check( false, "only uppercase letters allowed in symbol_code string" );
            }
            value <<= 8;
            value |= *itr;
         }
   ...
   inline DataStream& operator<<(DataStream& ds, const uosio::symbol_code sym_code) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, uosio::symbol_code& sym_code) {
     uint64_t raw = 0;
   ...
   public:
      ...
      void print( bool show_precision = true )const {
         if( show_precision ){
            ::uosio::print( static_cast<uint64_t>(precision()), "," );
         }
         char buffer[7];
         auto end = code().write_as_string( buffer, buffer + sizeof(buffer) );
         if( buffer < end )
            ::uosio::print( buffer, (end-buffer) );
      }
    ...
    */
   template<typename DataStream>
   inline DataStream& operator<<(DataStream& ds, const uosio::symbol sym) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, uosio::symbol& sym) {
     uint64_t raw = 0;
     ds.read((char*)&raw, sizeof(raw));
     sym = symbol(raw);
     return ds;
   ...
   private:
      ...
      UOSLIB_SERIALIZE( extended_symbol, (symbol)(contract) )
   };
}
