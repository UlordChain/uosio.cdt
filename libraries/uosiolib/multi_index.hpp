...
namespace _multi_index_detail {
   ...
   struct secondary_key_traits<uosio::fixed_bytes<32>> {
      static constexpr uosio::fixed_bytes<32> true_lowest() { return uosio::fixed_bytes<32>(); }
   };

}

/**
 *  The indexed_by struct is used to instantiate the indices for the Multi-Index table. In UOSIO, up to 16 secondary indices can be specified.
 *  @brief The indexed_by struct is used to instantiate the indices for the Multi-Index table. In UOSIO, up to 16 secondary indices can be specified.
 *
 *  @tparam IndexName - is the name of the index. The name must be provided as an UOSIO base32 encoded 64-bit integer and must conform to the UOSIO naming requirements of a maximum of 13 characters, the first twelve from the lowercase characters a-z, digits 1-5, and ".", and if there is a 13th character, it is restricted to lowercase characters a-p and ".".
 *  @tparam Extractor - is a function call operator that takes a const reference to the table object type and returns either a secondary key type or a reference to a secondary key type. It is recommended to use the `uosio::const_mem_fun` template, which is a type alias to the `boost::multi_index::const_mem_fun`. See the documentation for the Boost `const_mem_fun` key extractor for more details.
 *
 *  Example:
       *
*
 ...
 *  class mycontract: uosio::contract {
 *    struct record {
 *       uint64_t    primary;
 *       uint128_t   secondary;
 ...
 *      }
 *  }
 *  UOSIO_DISPATCH( mycontract, (myaction) )
 *  @endcode
 */
 ...
 *  UOSIO Multi-Index table also supports up to 16 secondary indices. The type of the secondary indices could be any of:
 *  - uint64_t
 *  - uint128_t
 *  - double
 *  - long double
...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            constexpr static bool validate_index_name( uosio::name n ) {
               return n.value != 0 && n != uosio::name("primary"); // Primary is a reserve index name.
            }
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     uosio::check( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        uosio::check( ei != -1, "cannot decrement end iterator when the index is empty" );
                        prev_itr = secondary_index_db_functions<secondary_key_type>::db_idx_previous( ei , &prev_pk );
                        uosio::check( prev_itr >= 0, "cannot decrement end iterator when the index is empty" );
                     } else {
                        if( _item->__iters[Number] == -1 ) {
                           secondary_key_type temp_secondary_key;
                           auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(_idx->get_code().value, _idx->get_scope(), _idx->name(), _item->primary_key(), temp_secondary_key);
                        ...
                        uosio::check( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator require_find( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               auto lb = lower_bound( secondary );
               uosio::check( lb != cend(), error_msg );
               uosio::check( secondary == secondary_extractor_type()(*lb), error_msg );
               return lb;
            }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               uosio::check( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               uosio::check( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, uosio::name payer, Lambda&& updater ) {
               uosio::check( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               uosio::check( itr != cend(), "cannot pass end iterator to erase" );
            ...
            uosio::name get_code()const  { return _multidx->get_code(); }
      ...
      static constexpr auto transform_indices( ) {
         ...
         return hana::transform( indices_input_type(), [&]( auto&& idx ){
             ...
             return hana::make_tuple( hana::type_c<index<uosio::name::raw(static_cast<uint64_t>(idx_type::index_name)),
                                                         typename idx_type::secondary_extractor_type,
                                                         num_type::e::value, false> >,
                                      hana::type_c<index<uosio::name::raw(static_cast<uint64_t>(idx_type::index_name)),
                                                         typename idx_type::secondary_extractor_type,
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         uosio::check( size >= 0, "error reading iterator" );
   ...
   public:
       ...
       *
       *  Notes
       *  The `uosio::multi_index` template has template parameters `<name::raw TableName, typename T, typename... Indices>`, where:
       *  - `TableName` is the name of the table, maximum 12 characters long, characters in the name from the set of lowercase letters, digits 1 to 5, and the "." (period) character and is converted to a uosio::raw - which wraps uint64_t;
       ...
       *  - It is recommended to use the uosio::const_mem_fun template, which is a type alias to the boost::multi_index::const_mem_fun.  See the documentation for the Boost const_mem_fun key extractor for more details.
       *
       *  Example:
       *
       *  @code
       ...
       *      typedef uosio::multi_index< "address"_n, address > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self.value); // code, scope
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       */
       ...
       *        uosio::check(addresses.get_code() == "dan"_n, "Codes don't match.");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *      void myaction() {
       *        address_index addresses("dan"_n, "dan"_n); // code, scope
       *        uosio::check(addresses.get_code() == "dan"_n, "Scopes don't match");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               uosio::check( ei != -1, "cannot decrement end iterator when the table is empty" );
               prev_itr = db_previous_i64( ei , &prev_pk );
               uosio::check( prev_itr >= 0, "cannot decrement end iterator when the table is empty" );
            } else {
               prev_itr = db_previous_i64( _item->__primary_itr, &prev_pk );
               uosio::check( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
       ...
       *        uosio::check(itr == addresses.cbegin(), "Only address is not at front.");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        uosio::check(itr == addresses.begin(), "Only address is not at front.");
       *      }
       *  }
       *  UOSIO_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator begin()const  { return cbegin(); }
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        uosio::check(itr != addresses.cend(), "Address for account doesn't exist");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        uosio::check(itr != addresses.end(), "Address for account doesn't exist");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *          address.street = "1 UOS Way";
       *          address.city = "Hong Kong";
       *          address.state = "HK";
       *        });
       *        auto itr = addresses.crbegin();
       *        uosio::check(itr->account_name == name("dan"), "Lock arf, Incorrect Last Record ");
       *        itr++;
       *        uosio::check(itr->account_name == name("brendan"), "Lock arf, Incorrect Second Last Record");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        itr--;
       *        uosio::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Record ");
       *        itr--;
       *        uosio::check(itr->account_name == name("dan"), "Lock arf, Incorrect Second Record");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        uosio::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Lower Bound Record ");
       *        itr++;
       *        uosio::check(itr->account_name == name("dan"), "Lock arf, Incorrect Second Lower Bound Record");
       *        itr++;
       *        uosio::check(itr == zip_index.end(), "Lock arf, Incorrect End of Iterator");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        uosio::check(itr->account_name == name("dan"), "Lock arf, Incorrect First Upper Bound Record ");
       *        itr++;
       *        uosio::check(itr == zip_index.end(), "Lock arf, Incorrect End of Iterator");
       *      }
       *  }
      ...
      uint64_t available_primary_key()const {
         ...
         uosio::check( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
       ...
       *      typedef uosio::multi_index< name("address"), address, indexed_by< name("zip"), const_mem_fun<address, uint64_t, &address::by_zip> > address_index;
       *      void myaction() {
       *        // create reference to address_index  - see emplace example below
       *        // add dan account to table           - see emplace example below
       ...
       *        uosio::check(itr->account_name == name("dan"), "Lock arf, Incorrect Record ");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        uosio::check(iterator_to(user) == itr, "Invalid iterator");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         uosio::check( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      /**
      ...
      const_iterator emplace( name payer, Lambda&& constructor ) {
         ...
         uosio::check( _code.value == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        uosio::check(itr != addresses.end(), "Address for account not found");
      ...
      void modify( const_iterator itr, name payer, Lambda&& updater ) {
         uosio::check( itr != end(), "cannot pass end iterator to modify" );
       ...
       *          address.state = "CA";
       *        });
       *        uosio::check(itr->city == "San Luis Obispo", "Lock arf, Address not modified");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, name payer, Lambda&& updater ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         uosio::check( objitem.__idx == this, "object passed to modify is not in multi_index" );
         auto& mutableitem = const_cast<item&>(objitem);
         uosio::check( _code.value == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         uosio::check( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
       ...
       *
       *        auto user = addresses.get("dan"_n);
       *        uosio::check(user.first_name == "Daniel", "Couldn't get him.");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        uosio::check(itr != addresses.end(), "Couldn't get him.");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        uosio::check(itr != addresses.end(), "Address for account not found");
       *        addresses.erase( itr );
       *        uosio::check(itr != addresses.end(), "Everting lock arf, Address not erased properly");
       *      }
       *  }
       *  UOSIO_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator erase( const_iterator itr ) {
         uosio::check( itr != end(), "cannot pass end iterator to erase" );
       ...
       *      void myaction() {
       *        auto itr = addresses.find("dan"_n);
       *        uosio::check(itr != addresses.end(), "Record is not found");
       *        addresses.erase(*itr);
       *        itr = addresses.find("dan"_n);
       *        uosio::check(itr == addresses.end(), "Record is not deleted");
       *      }
       *  }
       *  UOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      void erase( const T& obj ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         uosio::check( objitem.__idx == this, "object passed to erase is not in multi_index" );
         uosio::check( _code.value == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         uosio::check( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
