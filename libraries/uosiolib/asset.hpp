...
#warning "<uosiolib/asset.hpp> is deprecated use <uosio/asset.hpp>"
...
namespace uosio {
   ...
   */
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         uosio::check( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         uosio::check( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         uosio::check( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         uosio::check( -max_amount <= amount, "subtraction underflow" );
         uosio::check( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         uosio::check( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         uosio::check( -max_amount <= amount, "addition underflow" );
         uosio::check( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         uosio::check( tmp <= max_amount, "multiplication overflow" );
         uosio::check( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         uosio::check( a != 0, "divide by zero" );
         uosio::check( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         uosio::check( b.amount != 0, "divide by zero" );
         uosio::check( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
       ...
       */
      void print()const {
         uosio::print(to_string());
      }

      UOSLIB_SERIALIZE( asset, (amount)(symbol) )
   };
   ...
   */
   struct extended_asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         uosio::check( a.contract == b.contract, "type mismatch" );
      ...
      UOSLIB_SERIALIZE( extended_asset, (quantity)(contract) )
   };
