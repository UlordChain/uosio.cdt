...
namespace uosio {
    ...
    *    int  c; //4
    *
    *    UOSLIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   */
   ...
   struct permission_level {
      ...
      UOSLIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      UOSLIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         uosio::check( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
    ...
    * transfer_act{"uosio.token"_n, {st.issuer, "active"_n}}.send(st.issuer, to, quantity, memo);
    * // or
    * transfer_act trans_action{ "uosio.token"_n, {st.issuer, "active"_n}};
    * trans_action.send(st.issuer, to, quantity, memo);
    * @endcode
    */
   template <uosio::name::raw Name, auto Action>
   struct action_wrapper {
      template <typename Code>
      constexpr action_wrapper(Code&& code, std::vector<uosio::permission_level>&& perms)
         : code_name(std::forward<Code>(code)), permissions(std::move(perms)) {}

      template <typename Code>
      constexpr action_wrapper(Code&& code, const std::vector<uosio::permission_level>& perms)
         : code_name(std::forward<Code>(code)), permissions(perms) {}

      template <typename Code>
      constexpr action_wrapper(Code&& code, uosio::permission_level&& perm)
         : code_name(std::forward<Code>(code)), permissions({1, std::move(perm)}) {}

      template <typename Code>
      constexpr action_wrapper(Code&& code, const uosio::permission_level& perm)
      ...
      static constexpr uosio::name action_name = uosio::name(Name);
      uosio::name code_name;
      std::vector<uosio::permission_level> permissions;
   ...
   template <uosio::name::raw Name, auto... Actions>
   struct variant_action_wrapper {
      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, std::vector<uosio::permission_level>&& perms)
         : code_name(std::forward<Code>(code)), permissions(std::move(perms)) {}

      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, const std::vector<uosio::permission_level>& perms)
         : code_name(std::forward<Code>(code)), permissions(perms) {}

      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, uosio::permission_level&& perm)
         : code_name(std::forward<Code>(code)), permissions({1, std::move(perm)}) {}

      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, const uosio::permission_level& perm)
         : code_name(std::forward<Code>(code)), permissions({1, perm}) {}
...
::uosio::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::uosio::name(#NAME) )
 ...
 * UOSIO supports two communication models, inline and deferred. Inline actions are executed as part of the current transaction. This macro
 ...
 * @endcode
 * 
 * The example above is taken from uosio.token. 
 * This example:  
 *       uses the passed in, dereferenced `this` pointer, to call this.get_self() i.e. the uosio.token contract; 
 *       calls the uosio.token::transfer() action; 
