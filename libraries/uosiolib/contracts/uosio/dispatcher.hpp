...
namespace uosio {
   ...
   */
    ...
    * ```
    *
    * For this to work the Actions must be derived from uosio::contract
    *
    * @ingroup dispatcher
    *
    */
   ...
   bool dispatch( uint64_t code, uint64_t act ) {
      ...
      return uosio::dispatch<Contract,SecondAction,Actions...>( code, act );
   }
    ...
    *
    * @ingroup dispatcher
    * @tparam T - The contract class that has the correponding action handler, this contract should be derived from uosio::contract
 ...
 #define UOSIO_DISPATCH_INTERNAL( r, OP, elem ) \
    case uosio::name( BOOST_PP_STRINGIZE(elem) ).value: \
       uosio::execute_action( uosio::name(receiver), uosio::name(code), &OP::elem ); \
       break;
 ...
 *
 * @ingroup dispatcher
 * @note To be able to use this macro, the contract needs to be derived from uosio::contract
 * @param TYPE - The class name of the contract
 * @param MEMBERS - The sequence of available actions supported by this contract
 *
 * Example:
 * @code
 * UOSIO_DISPATCH( uosio::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
