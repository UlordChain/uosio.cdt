...
BOOST_FIXTURE_TEST_CASE( simple_tests, tester ) try {
   create_accounts( { N(test), N(uosio.token), N(someone), N(other) } );
   produce_block();

   set_code( N(uosio.token), contracts::transfer_wasm() );
   set_abi(  N(uosio.token),  contracts::transfer_abi().data() );
...
BOOST_FIXTURE_TEST_CASE( simple_uosio_tests, tester ) try {
   set_code( N(uosio), contracts::simple_wasm() );
   set_abi( N(uosio),  contracts::simple_wrong_abi().data() );
   produce_blocks();
   push_action(N(uosio), N(test1), N(uosio), 
         mvo()
         ("nm", "bucky"));

   BOOST_CHECK_THROW(push_action(N(uosio), N(test1), N(uosio), mvo()("nm", "notbucky")), 
         fc::exception);

   push_action(N(uosio), N(test2), N(uosio), 
         mvo()
         ("arg0", 33)
         ("arg1", "some string"));
   BOOST_CHECK_THROW(push_action(N(uosio), N(test2), N(uosio), mvo() ("arg0", 30)("arg1", "some string")), fc::exception);
   BOOST_CHECK_THROW(push_action(N(uosio), N(test2), N(uosio), mvo() ("arg0", 33)("arg1", "not some string")), fc::exception);
   
   push_action(N(uosio), N(test3), N(uosio), 
         mvo()
         ("arg0", 33)
         ("arg1", "some string"));
