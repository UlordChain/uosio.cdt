...
#include <uosio/transaction.hpp>

#include "transfer.hpp" 

using namespace uosio;

class [[uosio::contract]] simple_tests : public contract {
   public:
      ...
      [[uosio::action("test4")]] 
      void test4(name to) {
         transfer_contract::transfer_action trans("uosio.token"_n, {_self, "active"_n});
      ...
      void on_transfer(name from, name to, asset quant, std::string memo) {
         check(get_first_receiver() == "uosio.token"_n, "should be uosio.token");
      ...
      void on_transfer2(name from, name to, asset quant, std::string memo) {
         check(get_first_receiver() != "uosio.token"_n, "should not be uosio.token");
         print_f("On notify 2 : % % % %", from, to, quant, memo);
      }

      [[uosio::on_notify("*::transfer2")]] 
...
extern "C" void post_dispatch(name self, name original_receiver, name action) {
   ...
   check(self == "uosio"_n, "should only be uosio for action failures");
}
