...
#include <uosio/binary_extension.hpp>
#include <uosio/crypto.hpp>
#include <uosio/datastream.hpp>
#include <uosio/ignore.hpp>
#include <uosio/symbol.hpp>
...
using uosio::binary_extension;
using uosio::datastream;
using uosio::fixed_bytes;
using uosio::ignore;
using uosio::ignore_wrapper;
using uosio::pack;
using uosio::pack_size;
...
struct be_test {
   ...
   be_test(int i) : val{i} {}
   int val;
   UOSLIB_SERIALIZE( be_test, (val) )
};

// Definitions in `uosio.cdt/libraries/uosio/datastream.hpp`
UOSIO_TEST_BEGIN(datastream_test)
   silence_output(true);
...
// Definitions in `uosio.cdt/libraries/uosio/datastream.hpp`
UOSIO_TEST_BEGIN(datastream_specialization_test)
   silence_output(true);
...
// Definitions in `uosio.cdt/libraries/uosio/datastream.hpp`
UOSIO_TEST_BEGIN(datastream_stream_test)
   silence_output(true);
...
// Definitions in `uosio.cdt/libraries/uosio/datastream.hpp`
UOSIO_TEST_BEGIN(misc_datastream_test)
   silence_output(true);
...
int main(int argc, char* argv[]) {
   UOSIO_TEST(datastream_test);
   UOSIO_TEST(datastream_specialization_test);
   UOSIO_TEST(datastream_stream_test);
   UOSIO_TEST(misc_datastream_test);
   return has_failed();
}
