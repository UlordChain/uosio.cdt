...
#include <uosio/rope.hpp>
#include <uosio/tester.hpp>
#include <string>

using namespace uosio::native;

UOSIO_TEST_BEGIN(rope_test)
   silence_output(false);
   uosio::rope r("test string 0");
   r += ", test string 1";
   r += ", test string 2";
   r += ", test string 3";
   r += ", test string 4";
   ...
   uosio::rope r2 = r + uosio::rope("lhs") + uosio::rope("rhs") + "some string";
   std::string s2 = s + std::string("lhs") + std::string("rhs") + "some string";

   r2 += uosio::rope("rvalue +=");
