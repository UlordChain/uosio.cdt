...
// Definitions in `uosio.cdt/libraries/uosio/symbol.hpp`
UOSIO_TEST_BEGIN(symbol_code_type_test)
   silence_output(true);
...
// Definitions in `uosio.cdt/libraries/uosio/symbol.hpp`
UOSIO_TEST_BEGIN(symbol_type_test)
   silence_output(true);
...
// Definitions in `uosio.cdt/libraries/uosio/symbol.hpp`
UOSIO_TEST_BEGIN(extended_symbol_type_test)
   silence_output(true);
...
int main(int argc, char* argv[]) {
   UOSIO_TEST(symbol_code_type_test);
   UOSIO_TEST(symbol_type_test);
   UOSIO_TEST(extended_symbol_type_test);
   return has_failed();
}
