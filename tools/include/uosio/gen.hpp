...
struct generation_utils {
      ...
      auto check = [&](const clang::Type* pt) {
        if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
         if (auto rt = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
            return rt->getDecl()->isUosioIgnore();
      ...
      auto get = [&](const clang::Type* pt) {
         if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
            if (auto decl = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
               return decl->getDecl()->isUosioIgnore() ? tst->getArg(0).getAsType() : type;
         return type;
      };
   ...
   static inline bool has_uosio_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->hasUosioRicardian();
   }
   static inline bool has_uosio_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->hasUosioRicardian();
   }

   static inline std::string get_uosio_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->getUosioRicardianAttr()->getName();
   }
   static inline std::string get_uosio_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->getUosioRicardianAttr()->getName();
   }

   static inline std::string get_action_name( const clang::CXXMethodDecl* decl ) {
      std::string action_name = "";
      auto tmp = decl->getUosioActionAttr()->getName();
      if (!tmp.empty())
         return tmp;
   ...
   static inline std::string get_notify_pair( const clang::CXXMethodDecl* decl ) {
      ...
      auto tmp = decl->getUosioNotifyAttr()->getName();
      return tmp;
   }
   ...
   static inline bool is_uosio_contract( const clang::CXXMethodDecl* decl, const std::string& cn ) {
      std::string name = "";
      if (decl->isUosioContract())
         name = decl->getUosioContractAttr()->getName();
      else if (decl->getParent()->isUosioContract())
         name = decl->getParent()->getUosioContractAttr()->getName();
      if (name.empty()) {
   ...
   static inline bool is_uosio_contract( const clang::CXXRecordDecl* decl, const std::string& cn ) {
      std::string name = "";
      auto pd = llvm::dyn_cast<clang::CXXRecordDecl>(decl->getParent());
      if (decl->isUosioContract()) {
         auto nm = decl->getUosioContractAttr()->getName().str();
         name = nm.empty() ? decl->getName().str() : nm;
      }
      else if (pd && pd->isUosioContract()) {
         auto nm = pd->getUosioContractAttr()->getName().str();
