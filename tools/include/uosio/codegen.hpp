...
#include <uosio/gen.hpp>

#include <uosio/utils.hpp>
#include <uosio/whereami/whereami.hpp>
#include <uosio/abi.hpp>
...
namespace uosio { namespace cdt {
   ...
   struct codegen_exception : public std::exception {
      virtual const char* what() const throw() {
         return "uosio.codegen fatal error";
      }
   };
   ...
   // remove after v1.7.0
   bool has_uosiolib = false;

   class uosio_ppcallbacks : public PPCallbacks {
      public:
         uosio_ppcallbacks(SourceManager& sm, std::string file) : sources(sm), fn(file) {}
      protected:
         virtual void InclusionDirective(
            ...
            if ( file_name.find("uosiolib") != StringRef::npos )
               has_uosiolib = true;
         }
   ...
   class uosio_codegen_visitor : public RecursiveASTVisitor<uosio_codegen_visitor>, public generation_utils {
      private:
      ...
      public:
         ...
         explicit uosio_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class uosio::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         */
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_uosio_contract(decl, cg.contract_name)) {
               if (has_uosiolib) {
                  ss << "\n\n#include <uosiolib/datastream.hpp>\n";
                  ss << "#include <uosiolib/name.hpp>\n";
               } else {
                  ss << "\n\n#include <uosio/datastream.hpp>\n";
                  ss << "#include <uosio/name.hpp>\n";
               }
               ss << "extern \"C\" {\n";
               ...
               ss << "}\n";
               ss << "uosio::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
               ...
               ss << decl->getParent()->getQualifiedNameAsString() << "{uosio::name{r},uosio::name{c},ds}." << decl->getNameAsString() << "(";
         ...
         void create_action_dispatch(CXXMethodDecl* decl) {
            ...
            create_dispatch("uosio_wasm_action", "__uosio_action_", func, decl);
         }

         void create_notify_dispatch(CXXMethodDecl* decl) {
            auto func = [](CXXMethodDecl* d) { return generation_utils::get_notify_pair(d); };
            create_dispatch("uosio_wasm_notify", "__uosio_notify_", func, decl);
         }
         ...
         virtual bool VisitCXXMethodDecl(CXXMethodDecl* decl) {
            ...
            if (decl->isUosioAction()) {
               name = generation_utils::get_action_name(decl);
               validate_name(name, [&]() {emitError(*ci, decl->getLocation(), "action not a valid uosio name");});
      ...
      class uosio_codegen_consumer : public ASTConsumer {
      private:
         uosio_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit uosio_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new uosio_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe) {
               ...
               try {
                  ...
                  std::stringstream ss;
                  ss << "extern \"C\" {\n";
                  ss << "void uosio_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, uosio_wasm_entry, uosio_wasm_abi(";
                  std::string abi = cg.abi;
                  ss << "\"" << _quoted(abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_uosio_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
                  ss << "uosio_assert_code(false, 1);";
                  ss << "}\n";
                  ss << "}";
      ...
      class uosio_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(_make_unique<uosio_ppcallbacks>(CI.getSourceManager(), file.str()));
            return _make_unique<uosio_codegen_consumer>(&CI, file);
         }
   };
