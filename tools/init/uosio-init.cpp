...
struct project {
   ...
   const std::string hpp = "#include <uosiolib/uosio.hpp>\n"
                           "using namespace uosio;\n\n"
   ...
   const std::string cmake = "project(@)\n\n"
                             "set(UOSIO_WASM_OLD_BEHAVIOR \"Off\")\n"
   ...
   const std::string cmake_extern = "include(ExternalProject)\n"
                                    ...
                                    "if(UOSIO_CDT_ROOT STREQUAL \"\" OR NOT UOSIO_CDT_ROOT)\n"
                                    "   find_package(uosio.cdt)\n"
                                    "endif()\n\n"
                                    "ExternalProject_Add(\n"
                                    "   @_project\n"
                                    ...
                                    "   CMAKE_ARGS -DCMAKE_TOOLCHAIN_FILE=${UOSIO_CDT_ROOT}/lib/cmake/uosio.cdt/UosioWasmToolchain.cmake\n"
                                    "   UPDATE_COMMAND \"\"\n"
                                    "   PATCH_COMMAND \"\"\n"
                                    "   TEST_COMMAND \"\"\n"
                                    "   INSTALL_COMMAND \"\"\n"
   ...
   const std::string readme_cmake = "--- @ Project ---\n\n"
                                    ...
                                    "   - You can then do a 'set contract' action with 'cluos' and point in to the './build/@' directory\n\n"
                                    " - Additions to CMake should be done to the CMakeLists.txt in the './src' directory and not in the top level CMakeLists.txt";

   const std::string readme_bare = " --- @ Project ---\n\n"
                                   " - How to Build -\n"
                                   "   - run the command 'uosio-cpp -abigen -o @.wasm @.cpp'\n";
...
int main(int argc, const char **argv) {
   ...
   cl::SetVersionPrinter([](llvm::raw_ostream& os) {
        os << "uosio-init version " << "@VERSION_FULL@" << "\n";
  });
   cl::OptionCategory cat("uosio-init", "generates an uosio smart contract project");
   ...
   cl::ParseCommandLineOptions(argc, argv, std::string("uosio-proj"));
   try {
