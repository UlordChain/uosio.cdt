...
// RUN: env IPHONUOS_DEPLOYMENT_TARGET=11.0 \
// RUN:   %clang -target armv7-apple-darwin -c -### %s 2> %t.err
// RUN:   FileCheck --input-file=%t.err --check-prefix=CHECK-VERSION-IOS4 %s
// CHECK-VERSION-IOS4: invalid iOS deployment version 'IPHONUOS_DEPLOYMENT_TARGET=11.0'
...
// RUN: env MACOSX_DEPLOYMENT_TARGET=10.5 IPHONUOS_DEPLOYMENT_TARGET=2.0 \
...
// RUN: env IPHONUOS_DEPLOYMENT_TARGET=2.3.1 \
// RUN:   %clang -target armv6-apple-darwin9 -c %s -### 2>&1 | \
...
// RUN: env IPHONUOS_DEPLOYMENT_TARGET=9.1 \
...
// RUN: env IPHONUOS_DEPLOYMENT_TARGET=10.1 \
