...
llvm::Constant *CodeGenModule::GetOrCreateLLVMFunction(
  ...
  if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
     if (FD->hasAttr<UosioWasmImportAttr>())
        isWasmImport = true;
     if (FD->hasAttr<UosioWasmEntryAttr>())
        isWasmEntry = true;
     if (FD->hasAttr<UosioWasmABIAttr>())
        isWasmABI = true;
     if (FD->hasAttr<UosioWasmActionAttr>())
        isWasmAction = true;
     if (FD->hasAttr<UosioWasmNotifyAttr>())
        isWasmNotify = true;
  ...
  if (isWasmImport)
   F->addFnAttr("uosio_wasm_import", "true");

  if (isWasmEntry)
   F->addFnAttr("uosio_wasm_entry", "true");
     
  if (isWasmABI)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("uosio_wasm_abi", FD->getWasmABI().c_str());
     }
  if (isWasmAction)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("uosio_wasm_action", FD->getUosioWasmAction().c_str());
     }
  if (isWasmNotify)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("uosio_wasm_notify", FD->getUosioWasmNotify().c_str());
     }
