...
namespace {
  // UosioApply - Mutate the apply function as needed 
  struct UosioApplyPass : public FunctionPass {
    static char ID; 
    UosioApplyPass() : FunctionPass(ID) {}
    bool runOnFunction(Function &F) override {
       if (F.hasFnAttribute("uosio_wasm_entry") || F.getName().equals("apply")) {
...
char UosioApplyPass::ID = 0;
static RegisterPass<UosioApplyPass> X("apply_fixup", "Uosio Apply Fixups");

static void registerUosioApplyPass(const PassManagerBuilder&, legacy::PassManagerBase& PM) { PM.add(new UosioApplyPass()); }
static RegisterStandardPasses RegisterMyPass(PassManagerBuilder::EP_EarlyAsPossible, registerUosioApplyPass);
