...
### uosiolib
#### Partitioning
In `uosio.cdt` v1.6.0, `uosiolib` will now be partitioned into 3 groups. These allow for finer grained allowance for particular modes of compilation.
  - CAPI
  - Contracts
  - Core
  - The `CAPI` will only be available to contract writers using `uosio-cc` and purely for `C` smart contracts.
  - The `Contracts` will be available to `uosio-cpp` for smart contract writing. 
  - `Core` will be available to `uosio-cpp` for any of the modes (present and future).  
  - `uosio-cpp` in `fnative` mode will have access to all of these groups.

To access these new partitioned header files, use `<uosio/header_file.hpp>` instead of `<uosiolib/header_file.hpp>`.  Please note that all the old header files are still available at the old `uosiolib` directory, but these are deprecated and will be removed in v1.7.0.  Also, once you change one header file from `uosiolib` to `uosio` you will need to do so for all other occurrences at that point, because of some conflicts with the auto generated dispatcher.
...
This entire API is now only available to C developers (i.e. using `uosio-cc`), and all internal uses in `Core` and `Contracts` have been guarded behind the namespace `uosio::internal_use_do_not_use`.  

### uosiolib Contracts API
- `action.hpp`
...
- `print.hpp`
   ...
   - removed explicit `print` overloads for `uosio::symbol_code`, `uosio::fixed_bytes<T>` from here.
- `rope.hpp`
   - new file
...
If the dispatcher is in notification handling mode and if your contract receives an `uosio::onerror` notification, then the contract will assert with an error code. You can circumvent this check if you explicitly supply an error handler for it ([[uosio::on_notify("uosio::onerror")]]).
